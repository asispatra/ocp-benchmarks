...
if echo "$@" | grep workload-wp-tlbflush > /dev/null; then 
  ...
  #sed -i 's/PROCESSES=4/PROCESSES=16/g' mmtests_CI/configs/config-workload-wp-tlbflush 
...
echo $! > PID
bash getmem_data.sh > KMEM.log 2>&1 &
echo $! >> PID
...
#cat KMEM.log
echo "* * * * * KMEM.log(Bytes)  * * * *"
cat KMEM.log | grep "### CONTAINER CGROUP DIR:"
echo "### KMEM: ONLY ACTUAL RUN"
for PATTERN in "### MEM WORKER:" "### MEM kubepods:" "### MEM kubepods-.*pod" "### MEM crio-" "### KMEM WORKER:" "### KMEM kubepods:" "### KMEM kubepods-.*pod" "### KMEM crio-" ; do
  cat KMEM.log | grep '+++' | grep "$PATTERN" | sed 's/.*: \(.*\)/\1/g' | jq -s 'max'
done | paste -s -d ' '
...
for PATTERN in "### MEM WORKER:" "### MEM kubepods:" "### MEM kubepods-.*pod" "### MEM crio-" "### KMEM WORKER:" "### KMEM kubepods:" "### KMEM kubepods-.*pod" "### KMEM crio-" ; do
  cat KMEM.log | grep -e '+++' -e '===' | grep "$PATTERN" | sed 's/.*: \(.*\)/\1/g' | jq -s 'max'
done | paste -s -d ' '
